...
class TextChatClient:
    ...
    def generate_text(
        ...
        **kwargs
        ...
        stream_mode = self.stream_enabled if stream is None else stream
        ...
        for line in response.iter_lines(decode_unicode=True):
            ...
            for choice in chunk.get("choices", []):
                ...
                # 同时兼容 reasoning_content 与 content 字段
        ...
        if not final_text:
            raise Exception("流式响应未返回任何文本内容，可能是兼容接口未正确返回 data: 消息")

        return final_text

    def _raise_for_status(self, response: requests.Response, model: str):
