        ...
        <div class="modal-body">
          ...
          <div class="form-group">
            <label class="toggle-label">
              <span>启用流式 (stream)</span>
              <div
                class="toggle-switch"
                :class="{ active: textProviderForm.stream }"
                @click="textProviderForm.stream = !textProviderForm.stream"
              >
                <div class="toggle-slider"></div>
              </div>
            </label>
            <span class="form-hint">开启后以流式方式解析兼容接口（建议保持开启）。</span>
          </div>
        </div>
      ...
      <div class="modal-content" @click.stop>
        ...
        <div class="modal-body">
          ...
          <div class="form-group">
            <label class="toggle-label">
              <span>启用流式 (stream)</span>
              <div class="toggle-switch" :class="{ active: imageProviderForm.stream }" @click="imageProviderForm.stream = !imageProviderForm.stream">
                <div class="toggle-slider"></div>
              </div>
            </label>
            <span class="form-hint">针对需要流式返回图片链接的兼容接口（如 chat/completions），建议开启。</span>
          </div>
        </div>
        <div class="modal-footer">
...
async function saveTextProvider() {
  ...
  const providerData: any = {
    ...
    stream: textProviderForm.value.stream
  }
...
async function saveImageProvider() {
  ...
  const providerData: any = {
    ...
    stream: imageProviderForm.value.stream
  }
